<?php
/**
 * Beautiful Reader functions and definitions.
 *
...
if ( ! function_exists( 'beautiful_reader_setup' ) ) :
/**
 ...
 */
function beautiful_reader_setup() {
	/*
	 * Make theme available for translation.
	 * Translations can be filed in the /languages/ directory.
	 * If you're building a theme based on Beautiful Reader, use a find and replace
	 * to change 'beautiful_reader' to the name of your theme in all the template files.
	 */
	load_theme_textdomain( 'beautiful_reader', get_template_directory() . '/languages' );
	...
	add_theme_support( 'custom-background', apply_filters( 'beautiful_reader_custom_background_args', array(
		'default-color' => 'ffffff',
		'default-image' => '',
	) ) );
}
endif;
add_action( 'after_setup_theme', 'beautiful_reader_setup' );
 ...
 */
function beautiful_reader_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'beautiful_reader_content_width', 640 );
}
add_action( 'after_setup_theme', 'beautiful_reader_content_width', 0 );
...
function beautiful_reader_scripts() {
	...
	wp_enqueue_script( 'beautiful_reader-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20130115', true );
...
function beautiful_reader_comment($comment, $args, $depth) {
    ...
    <div class="comment-meta">
        ...
        <div class="comment-metadata commentmetadata"><a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ); ?>">
            <?php
                ...
                printf( __('%1$s at %2$s'), get_comment_date( 'M j, Y' ),  get_comment_time() ); ?></a><?php edit_comment_link( __( '(Edit)' ), '  ', '' );
            ?>
        </div>
    </div>
